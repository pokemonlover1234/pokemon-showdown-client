...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected firstLearnsetid(speciesid: ID) {
		...
		if (this.ionitemetas.includes(this.dex.modid)) table = table[this.dex.modid];
	...
	protected nextLearnsetid(learnsetid: ID, speciesid: ID, checkingMoves = false) {
		...
		if (this.ionitemetas.includes(this.dex.modid) && learnsetid !== 'rockruffdusk') {
			if (lsetSpecies.forme !== null && lsetSpecies.forme !== "" &&
				lsetSpecies.baseSpecies && toID(lsetSpecies.baseSpecies) !== learnsetid) {
